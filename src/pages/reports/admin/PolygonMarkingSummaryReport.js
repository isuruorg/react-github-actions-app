...
export default function PolygonMarkingSummaryReport() {
  ...
  const downloadReport = useCallback(async () => {
    try {
      ...
      const response = await axios.post('/reports/polygonMarkingSummary', data);
      if (response) {
        ...
        <Card sx={{ p: 3 }}>
          ...
          <Stack spacing={{ xs: 2, md: 3 }}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={{ xs: 3, sm: 2 }} sx={{ justifyContent: 'end' }}>
              <MButton color="primary" size="medium" variant="contained" onClick={() => downloadReport()}>
                Download
              </MButton>
            </Stack>
          </Stack>
